    ...
    T: Default,
{
    ...
    pub fn add(&mut self, value: T) {
        // Append at the end and percolate up
    ...
    fn smallest_child_idx(&self, idx: usize) -> usize {
        ...
        if right <= self.count {
            // both children present, choose the one preferred by comparator
    ...
    T: Default,
{
    ...
    fn next(&mut self) -> Option<T> {
        ...
        // remove and return the root (index 1)
        let top = self.items.swap_remove(1);
        self.count -= 1;

        // if there are still elements, percolate down the new root
        if self.count >= 1 {
            let mut idx = 1usize;
